...
## settings
do_these = ['Marid', 'Sea Spawn', 'Sea Spawn', 'Sea Spawn', 'Sea Spawn', 'Sea Spawn', 'Sea Spawn', 'Sea Spawn', 'Leviathan', 'Water Elemental', 'Water Elemental', 'Water Elemental', 'Storm Giant', 'Sea Hag', 'Sea Hag']
paper_format = 'a4'
...
def create_mini(monster):
    ...
    if m_img.shape[0] < min_height*dpmm:
        ...
        m_img = np.concatenate((np.zeros((top,m_img.shape[1],3), np.uint8)+255,m_img,np.zeros((top,m_img.shape[1],3), np.uint8)+255),axis=0)
    ...
    if m_img.shape[1] < width:
        ...
        right = left
        if diff%2 == 1: right += 1
        m_img = np.concatenate((np.zeros((m_img.shape[0],left,3), np.uint8)+255,m_img,np.zeros((m_img.shape[0],right,3), np.uint8)+255),axis=1)
        
    cv.rectangle(m_img, (0,0), (m_img.shape[1]-1,m_img.shape[0]-1), (0,0,0), thickness=1)
    
    ## create mini base
    demi_base = b_height//2*dpmm
    if creature_size == 'Gargantuan': feet_mod = 1
    else: feet_mod = 2
...
M = greedypacker.BinManager(canvas[0], canvas[1], pack_algo='shelf', heuristic='best_width_fit', wastemap=True, rotation=True)
its = {}
item_id = 0
for it in minis:
    its[item_id] = it
    item = greedypacker.Item(it.shape[1],it.shape[0],item_id)
    M.add_items(item)
    item_id += 1
...
sheet_nr = 1
for r in result:
    img = np.zeros((canvas[1],canvas[0],3), np.uint8)+255
    for it in r.minis:
        x = int(it.x)
        y = int(it.y)
        w = int(it.width)
